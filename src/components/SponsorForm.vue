<template>
  <div class="flex flex-col mx-6 mt-10 md:flex-row md:mx-10 sponsor-form">
    <div class="message-wrapper">
      <form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
        <input type="hidden" name="cmd" value="_s-xclick">
        <input type="hidden" name="hosted_button_id" value="XEHR2PK6LPGUC">
        <label class="block mb-6">
          <span class="text-gray-700">Select a topic  &#8628;</span>
          <select name="os1" class="block w-full mt-1 form-select">
              <option v-for="edge in $static.upcoming.edges" :key="edge.node.id">{{ edge.node.title }}</option>
          </select>
        </label>
...
.format sup {
  ...
  .sponsor main > div > div:last-of-type svg {
    position: absolute;
    top: 0;
    right: 0;
    opacity: .2;
...
.sponsor .main > div > div:last-of-type h3 {
  margin-top: 0;
}
.message-wrapper div:last-of-type > div {
  margin-top: .5rem;
}
.message-wrapper button mark {
    background-color: #fff;
    border-radius: 2px;
    padding: 0 6px;
}
.message-wrapper div:last-of-type > div label sup {
  color: #777;
  font-size: .8rem;
  font-weight: normal;
}
@media (min-width: 768px) {
  ...
  .sponsor .main > div > div {
    width: 70%;
  }
  .sponsor .main > div > div:last-of-type {
    width: 30%;
    background: #f8f8f8e5;
    position: relative;
    margin-left: 3rem;
