<template>
	<Layout :sidebar="true" :top="false" :bottom="true">
		...
		<p class="lead mt-6">Below is a list of the services I offer. These days I'm mostly focussed on building JAMstack sites, but I'm also open to design and theming projects.</p>
		<template slot="bottom-shelf">
			<div class="grid grid-cols-1 gap-8 px-6 mt-10 mb-16 md:grid-cols-2 md:gap-8 lg:grid-cols-3 md:px-10 lg:px-20">
				<div v-for="edge in $page.posts.edges" :key="edge.node.id">
					<div class="flex items-center mb-3 md:m-0 md:block">
						<figure v-if="edge.node.icon" class="w-8 md:w-10 lg:w-10">
							<g-image :src="edge.node.icon" width="10" height="10" :alt="edge.node.title" immediate="true" />
						</figure>
			...
			<p class="px-6 mb-12 md:px-10 lg:px-20">Available for new projects. <g-link to="/contact" class="no-underline hover:bg-yellow-100" title="Contact me to discuss your project">Enquire within &rarr;</g-link></p>
		</template>
